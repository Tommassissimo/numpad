...
function Keypad() {
	...
	this.addKeyDef('PLUSE','plus',function(inst){plugin._stepAdd(inst);});
	this.addKeyDef('MINUS','minus',function(inst){plugin._stepMinus(inst);});
	...
	this._defaults = { // Global defaults for all the keypad instances
		...
		layout: ['789'+this.PLUSE, '456'+this.MINUS, '123' + this.BACK, '.0'+this.CLOSE,], // Layout of keys
...
$.extend(Keypad.prototype, {
	...
	_disablePlugin: function(target) {
		...
		if (nodeName.match(/input|textarea/)) {
			...
			target.prepend('<div class="' + this._disableClass + '" style="left: ' + (offset.left - relOffset.left) +
	...
	_showPlugin: function(field) {
		console.log(new Date(),"-------->_showPlugin");
	...
	_updateKeypad: function(inst) {
				console.log(new Date(),"-------->_updateKeypad");
	...
	_checkOffset: function(inst, offset, isFixed) {
			console.log(new Date(),"--------> _checkOffset");
	...
	_hidePlugin: function(field, duration) {
		console.log(new Date(),"--------> __hidePlugin");
	...
	_doKeyDown: function(e) {
		console.log(new Date(),"--------> _doKeyDown");
	...
	_checkExternalClick: function(event) {
		console.log(new Date(),"----->_checkExternalClick");
		if (!plugin._curInst) {
			return;
		}
	...
	_getIERange: function(field) {
		console.log(new Date(),"--------> __getIERange");
		field.focus();
	...
	_stepAdd: function(inst){
		var maxlen = inst._input.attr('maxlength');
		if (maxlen > -1) {
			value = value.substr(0, maxlen);
		}
		var value = parseInt(inst._input.val());
		if(!value){value=0};
		inst._input.val(value+1);
		if (!$.isFunction(inst.options.onKeypress)) {
			inst._input.trigger('change'); // fire the change event
		}
	},
	_stepMinus: function(inst){
		var maxlen = inst._input.attr('maxlength');
		if (maxlen > -1) {
			value = value.substr(0, maxlen);
		}
	...
	_notifyKeypress: function(inst, key) {
		console.log(new Date(),"--------> _notifyKeypress");
	...
	console.log(new Date(),"--------> _generateHTML");
	...
	_randomiseLayout: function(inst) {
		console.log(new Date(),"--------> _randomiseLayout");
	...
	function isNotChained(command, otherArgs) {
		console.log(new Date(),"-------->isNotChained");
	...
	$.fn.keypad = function(options) {
		console.log(new Date(),"-------->$.fn.keypad");
...
console.log(new Date(),"-------->initiation");
